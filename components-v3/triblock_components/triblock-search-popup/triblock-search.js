...
import "./triblock-search-loading-no-matches.js";
...
Polymer({
    _template: html`
		<style include="iron-flex iron-flex-alignment iron-flex-factors iron-positioning tristyles-theme">
				...
				.results-dropdown {
					@apply --triblock-search-results-dropdown;
				}
		...
		<div class="search-content">
			...
			<paper-input-container id="searchField" focused="{{focused}}" disabled\$="[[disabled]]" aria-label="{{label}}" no-label-float="">
				<iron-icon icon="ibm:search" prefix="" hidden="[[noSearchIcon]]" slot="prefix"></iron-icon>
				...
				<iron-input id="[[_inputId]]" bind-value="{{_searchValue}}" slot="input">
					<input placeholder="[[_computePlaceHolder(placeholder)]]" aria-labelledby="[[_labelId]]" disabled\$="[[disabled]]" aria-label\$="{{label}}" on-tap="_onInputTapped">
				</iron-input>
				<paper-icon-button class="icon-clear" icon="ibm-glyphs:clear-input" suffix="" on-tap="_clearSearch" hidden="[[_hideClearBtn]]" alt\$="[[altClearButton]]" slot="suffix"></paper-icon-button>
				<paper-icon-button class="icon-select" icon="ibm-glyphs:expand-close" suffix="" on-tap="_onIconSelectTapped" hidden="[[!selectBox]]" alt\$="[[altDropdownButton]]" slot="suffix"></paper-icon-button>
			</paper-input-container>
			...
			<template is="dom-if" if="[[dropdown]]">
				<iron-dropdown id="dropdown" horizontal-align="left" vertical-align="top" vertical-offset="0" position-target="[[_targetEl]]" scroller="{{scroller}}">
					<div id="dropdownContent" class="results-dropdown dropdown-content" slot="dropdown-content">
						<triblock-table id="resultsTable" data="{{searchResult}}" disable-default-tooltips="" on-row-tap="_selectItem" on-dom-change="_onResultsTableChanged" on-keydown="_onTableTabPressed" tabindex="-1" row-aria-label-callback="[[rowAriaLabelCallback]]">
							<triblock-table-column>
								<slot></slot>
							</triblock-table-column>
    ...
    listeners: {
		...
		"iron-overlay-canceled": "_handleOverlayCanceled"
	},
	...
	_setTableHeight: function(searchValue, searchResult) {
		...
		if (this.dropdown && this.$$("triblock-table")) {
			...
			this.async(function() {
				...
				var listHeight = this.$$("triblock-table").tableBody.getBoundingClientRect().height;
	...
	_handleOverlayCanceled: function(e) {
		// needed to stop propagation when this is inside a popup
		e.stopPropagation();
	}
