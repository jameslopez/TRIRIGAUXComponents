...
import { TriCompCalendarContainerBehavior } from "./triplat-calendar-container-behavior.js";
...
], "triplat-calendar-container/triplat-calendar-container.js");
...
*/
	Polymer({
		...
		<div id="picker" class="">
			<triplat-calendar id="calendar" header="{{_header}}" disabled\$="{{disabled}}" current-date="{{_dateValue}}" disallow-past-dates\$="{{disallowPastDates}}" disallow-weekends\$="{{disallowWeekends}}" restrict-to-date-list\$="{{restrictToDateList}}" valid-date-list="{{validDateList}}" no-cancel-on-esc-key="{{_noCancelOnEscKey}}" no-cancel-on-outside-click="{{_noCancelOnOutsideClick}}" width-of-parent="{{_widthOfParent}}" triplat-calendar-container="">
			</triplat-calendar>
		</div>
	`,

	    is: "triplat-calendar-container",
	    behaviors: [ TriCompCalendarContainerBehavior, IronResizableBehavior, TriDateUtilities ],
