...
import "./triplat-ds-search-dropdown.js";
import "./triplat-ds-search-filters-dropdown.js";
import "./triplat-ds-search-context.js";
/*
    ...
    _template: html`
		...
		#inputContainer {
			...
			padding-left: 5px;
				...
				border-bottom: var(--triplat-ds-search-input-border-height, 2px) solid var(--triplat-ds-search-input-border-color, var(--tri-primary-color));
			height: 44px;
			...
			.search-icon {
				...
				min-height: 22px;
				padding: 11px;
				color: var(--triplat-ds-search-input-search-icon-color, var(--tri-primary-content-label-color));
			}
			...
			.clear-icon {
				...
				min-height: 22px;
				padding: 11px;
				color: var(--triplat-ds-search-input-clear-icon-color, var(--tri-primary-content-label-color));
				cursor: pointer;
			}
		...
		.filters-icon {
				min-width: 24px; 
				min-height: 24px;
				color: var(--triplat-ds-search-input-filters-icon-color, var(--tri-primary-color));
			}
		
		iron-icon.filters-expand-icon {
				width: 16px; 
				height: 16px;
				color: var(--triplat-ds-search-input-expand-filters-icon-color, var(--tri-primary-color));
				cursor: pointer;
			}
		...
		.filters-close-icon {
				width: 16px; 
				height: 16px;
				color: var(--triplat-ds-search-input-close-filters-icon-color, var(--tri-primary-color));
				cursor: pointer;
			}
		...
		.group {
			@apply --layout;
				@apply --layout-horizontal; 
			background-color: var(--triplat-ds-search-input-filter-background-color, var(--ibm-neutral-2));
		...
		.filters div div.clearfilter triplat-icon {
			...
			min-height: 24px;
			margin: 8px;
			color: var(--triplat-ds-search-input-filter-remove-icon-color, var(--tri-primary-content-label-color));
			cursor: pointer; 
		}
		...
		.filters-number-badge-text {
			font-family: var(--tri-font-family);
			font-size: 16px;
			color: var(--triplat-ds-search-input-expand-filters-number-color, var(--ibm-orange-50));
		...
		<div id="inputContainer">
			...
			<iron-input id="inputField" class="search-input" bind-value="{{_value}}">
				<input type="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" class="search-input" placeholder="{{_placeholder}}" aria-label\$="{{_placeholder}}" on-keydown="_handleKeyDown">
			</iron-input>
			...
			<template is="dom-if" if="{{_mobileView}}">
				<div class="filters-action">
					...
					<iron-icon class="filters-expand-icon" icon="ibm-glyphs:expand-open" tabindex="0" hidden\$="{{_filtersOpen}}" on-tap="_openFiltersMenu" description="Open all filters" on-keydown="_handleActionKeyDown" id="expandIcon"></iron-icon>
		...
		<triplat-ds-search-dropdown id="searchDropdown" value="{{_value}}" aliases="{{aliases}}" max-header-children="{{maxHeaderChildren}}" delay="{{delay}}" current-operator="{{_currentOperator}}">
		</triplat-ds-search-dropdown>
		<triplat-ds-search-filters-dropdown id="mobileFilters" filters="{{_filters}}"></triplat-ds-search-filters-dropdown>
	`,

    is: "triplat-ds-search-input",

    behaviors: [IronResizableBehavior,
				TriplatKeyboardBehavior],
