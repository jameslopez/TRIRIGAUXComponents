...
import "../fullcalendar/fullcalendar.css.js";
import "../fullcalendar/fullcalendar.print.css.js";
...
], "triplat-resource-calendar/triplat-resource-calendar.js");
...
*/
    Polymer({
	    _template: html`
		<style include="iron-flex iron-flex-alignment iron-flex-factors iron-positioning tristyles-theme full-calendar-styles">
		...
		.circledDay{
			...
			z-index: 0;
			background: var(--triplat-resource-calendar-circled-date-bg-color, var(--tri-primary-color-30));
		}
		.calendarTitle{
		...
		paper-icon-button.today-button {
			color: var(--triplat-resource-calendar-today-button-color, var(--tri-primary-button-background-color));
			padding-right: 5px;
		}
	...
	`,
		...
		*/
	    ...
	    attached:function(){
			...
			this.async(function(){
				this._calendar = $(dom(this.$.__triplatResourceCalendar).node);
		...
		*/
	    ...
	    _handleTodayColorChange: function(){
			...
			$(this.shadowRoot.querySelector(".fc-today")).css('background-color', resourceCalendarTodayColor);
		},
	    ...
	    _initializeComponentStyles: function(){
		    !this.getComputedStyleValue('--triplat-resource-calendar-resource-busy-unavailable');
			!this.getComputedStyleValue('--triplat-resource-calendar-resource-busy');
			!this.getComputedStyleValue('--triplat-resource-calendar-resource-busy');

			this.updateStyles({
			    "--triplat-resource-calendar-resource-busy-unavailable": '#B5B1B0',
			    "--triplat-resource-calendar-resource-busy": '#FF0000',
			    "--triplat-resource-calendar-resource-busy-tentative": '#FF5733'
			});
	    ...
	    _handleResourceOpacity: function(){
			...
			$(this.shadowRoot.querySelector('#__triplatResourceCalendar div.fc-bgevent')).css('opacity', bookedResourceOpacity);
			$(this.shadowRoot.querySelector('#__triplatResourceCalendar a.fc-time-grid-event.fc-v-event.fc-event.fc-start.fc-end')).css('opacity', bookingResourceOpacity);
		},
	    ...
	    _removeFCdays: function(){
			$(this.shadowRoot.querySelector('thead')).remove();
			$(this.shadowRoot.querySelector('.fc-toolbar')).remove();
		},
	    ...
	    _persistContentHeight: function(){
			...
			$(this.shadowRoot.querySelector('.fc-today')).removeClass('fc-state-highlight');
		},
