...
export const TriplatHighlightGroupLayers = {};
...
Polymer({
    ...
    _setMaskElement: function(svgLoaded) {
		...
		var maskGroup = dom(svgElement).querySelector("#_triMaskLayer");
		...
		dom(maskGroup).appendChild(rectElement);
    ...
    _createLinearGradient: function(space, legendSpace) {
		...
		dom(this.graphic.svgElement).appendChild(linearGradient);
    ...
    _clearAllLegendSpaces: function() {
		for(var i=1; i<=5; i++){
			Array.from(dom(this.graphic.svgElement).querySelectorAll("." + this._getClassName(i))).forEach(function(space) {
    ...
    _removeLegendSpace: function(id, classNumber) {
		...
		var space = this._getSpaceElement(id);
		 if (space.color) {
			dom(this.graphic.svgElement).removeChild(space.color);
			space.color = null;
		}
...
export const HighlightClass = Polymer({
