...
IBM.LMV.ForgeViewer = function() 
{
	...
	this.loadDocument = function( 
		...
		_selectionMgr.deferedSearch = _deferedSearch;
		_selectionMgr.deferedZoom   = _deferedZoom;
		_selectionMgr.deferedView   = _deferedView;
		_deferedSearch = null;
		_deferedZoom   = false;
		_deferedView   = null;
	...
	this.resize = function(
		...
		if( this.viewer && this.viewer.container )
		{
			...
			{
				this.viewer.resize();
				this.viewer.updatesetAutoZoomModesetAutoZoomModeButtons();
			}
	...
	this.displayView = function(
		...
		if( _selectionMgr )
		{
			_selectionMgr.displayView( view );
		}
		else
		{
			_deferedView = view;
...
IBM.LMV.SelectionMgr = function(
	...
	this.displayView = function(
		...
		{
			this.deferedView = view;
			return;
		}

		var state = JSON.parse( view );
		_forgeViewer.viewer.restoreState( state ); 
	}
	...
	this.doDeferedSearch = function()
	{
		...
		{
			_modelLoading = false;
			if( this.deferedView != null )
			{
				var state = JSON.parse( this.deferedView );
				this.deferedView = null;
				_forgeViewer.viewer.restoreState( state ); 
			}
			if( this.deferedSearch != null )
			{
				try
				{
					var search = this.deferedSearch;
			...
			else if( this.deferedZoom == "center" )
			{
				this.deferedZoom   = false;
				this.zoomIntoSelection();
			}
	...
	this.zoomIntoSelection = function()
	{
		if( this.deferedSearch )
		{
			this.deferedZoom = "center";
			return;
		}
