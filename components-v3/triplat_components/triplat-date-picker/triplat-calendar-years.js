...
const importJsPromise = importJs(
  ...
  "triplat-date-picker/triplat-calendar-years.js"
);
...
.selectedYear:after {
  ...
  left: 3px;
  z-index: 0;
  background-color: var(--triplat-date-picker-selected-bg-color, var(--tri-primary-color-10));
  opacity: 0.5;
}
...
.actionButton{
  color: var(--triplat-date-picker-button-color, var(--tri-primary-button-background-color));
...
paper-button.actionButton {
  ...
  background-color: rgb(255, 255, 255);
	  ...
	  border-style: solid;
	  border-color: var(--triplat-date-picker-button-color, var(--tri-primary-button-background-color));
	  border-width: 1px;
	}
...
<template is="dom-if" if="{{confirmButtons}}">
 <template is="dom-if" if="{{!triplatCalendarContainer}}">
  ...
  `,
    ...
    behaviors: [ TriPlatCalendarBehavior,
