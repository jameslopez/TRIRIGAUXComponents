...
import "./triplat-calendar.js";
import { TriPlatDatePickerBehavior } from "./triplat-date-picker-behavior.js";
...
], "triplat-date-picker/triplat-date-picker.js");
...
*/
    Polymer({
		...
		<paper-input-container hidden="{{_readonlyNoDisabledNoLabel(disabled,readonly,label)}}" class="flex" on-change="_directEdit" invalid="[[_invalid]]" aria-label\$="[[label]]" always-float-label\$="[[alwaysFloatLabel]]" disabled\$="[[disabled]]" no-label-float\$="[[noLabelFloat]]">
			<label id="[[_labelId]]" for\$="[[_inputId]]" slot="label">[[label]]<slot id="labelContent" name="label"></slot></label>
			<iron-input id="[[_inputId]]" bind-value\$="{{formatDate(value, displayFormat)}}" slot="input">
