	...
	Polymer({
		...
		_updateDisabled: function(newValue, oldValue) {
			...
			if ((typeof newValue != 'undefined') && (typeof oldValue != 'undefined')) {
				this.setupDragging(this.elements);
			}
		},
		...
		get elements() {
			return dom(this).querySelectorAll(this.selector);
		},
		...
		_setupDefaultOptions: function() {
			...
			var onStartHandler = function(event) {
				let triplatDrag = event.target.triplatDrag;
		...
		_setupClone: function(clone, selector, info) {
			...
			selectArray.forEach(function(s) {
				...
				if (_selector.slice(0, 1) == "#") {
					clone.id = _selector.substr(1);
				} else if (_selector.slice(0, 1) == ".") {
					clone.classList.add(_selector.substr(1));
				} else {
					console.error("unknown selector = " + selector + " in triplat-drag")
					return null;
				}
			});
			clone.setAttribute("data-info", info);
			clone.setAttribute("data-drag-id", this._containerId);
			clone.set("triplatDrag", this);
		},
	
		_createClone: function(original, selector, parent, event) {
			var clone = document.createElement("iron-icon");
			// search upward to find data-info value
			var info = this._triInteract.findAttributeValue(original, "data-info");
			
			clone.icon = this.icon;
			parent.appendChild(clone);
			this._setupClone(clone, selector, info);
			if (clone) {
				this._styleClone(clone, event);
			}
			return clone;
		},
		...
		_cloneMove: function(event) {
			...
			if (interaction.pointerIsDown && !interaction.interacting()) {
				...
				if (this._triInteract.getOption("enabled")) {
		 ...
		 */
		setupDragging: function(elements) {
