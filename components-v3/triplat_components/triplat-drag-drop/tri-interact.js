 ...
 * 
 * History
 * 12/04/2018: copy from interact.js v1.3.4 and rename to tri-interact.js
 */
(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.interact = f()}})(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
	...
	function getDrop(dragEvent, event, dragElement) {
	  ...
	  for (var j = 0; j < interaction.activeDrops.dropzones.length; j++) {
		...
		if (current.dropCheck(dragEvent, event, interaction.target, dragElement, currentElement, rect)) {
			if (current.options.drop.strategy) {
				var dropContainer = {};
				dropContainer['index'] = j;
				dropContainer['rect'] = rect;
				dropContainer['element'] = currentElement;
				dropContainer['svgContainer'] = current.options.drop.svgContainer;
				if (event.touches) {
				...
				dropContainer['clientX'] = clientX;
				dropContainer['clientY'] = clientY;
				validDrops.push(dropContainer);
				domHierarchyStratgy = false;
			}	
			else {
				validDrops.push(currentElement);
				domHierarchyStratgy = true;
			}
		} else {
			validDrops.push(null);
		}
	  }
	
		// get the most appropriate dropzone based on either DOM depth and order or rectangular containing
		var dropIndex;
		if (domHierarchyStratgy) {
			dropIndex = utils.indexOfDeepestElement(validDrops);
		} else {
			dropIndex = utils.indexOfDeepestElementUsingPolygon(validDrops);
		}
	...
	Interactable.prototype.dropzone = function (options) {
	  if (utils.is.object(options)) {
		...
		if ('strategy' in options) {
			this.options.drop.strategy = options.strategy;
		}
		if ('svgContainer' in options) {
			this.options.drop.svgContainer = options.svgContainer;
		}
	...
	*/
	...
	var domUtils = {
	  ...
	  indexOfDeepestElement: function indexOfDeepestElement(elements) {
		...
		indexOfDeepestElementUsingPolygon: function indexOfDeepestElementUsingPolygon(containers) {
				var dropzone,
						deepestZone = containers[0],
						index,
						i;
				var overlapZones = [];

				for (i = 1; i < containers.length; i++) {
						dropzone = containers[i];

						// an element might belong to multiple selector dropzones
						if (!dropzone || dropzone === deepestZone || !dropzone.svgContainer) {
								continue;
						}

						// Polygon is defined in polygon.js
						if (Polygon.pointInPolyline(dropzone.clientX, dropzone.clientY, dropzone.element, 
								dropzone.svgContainer.element, dropzone.svgContainer.position)) {
							deepestZone = dropzone;
							overlapZones.push(dropzone);
						}
								
				}
				if (overlapZones.length <= 1) {
					dropzone = overlapZones[0];
					if (dropzone) {
					...
					deepestZone = overlapZones[0];
					for (i = 1; i < overlapZones.length; i++) {
						dropzone = overlapZones[i];
						if (Polygon.recContains(deepestZone.rect.left, deepestZone.rect.top, deepestZone.rect.width, deepestZone.rect.height, 
									dropzone.rect.left, dropzone.rect.top, dropzone.rect.width, dropzone.rect.height)) {
							deepestZone = dropzone;
						}
					}
					return deepestZone.index;
				}
		},
	...
	var pointerUtils = {
	  ...
	  getPageXY: function getPageXY(pointer, page) {
		...
		if (browser.isOperaMobile && pointerUtils.isNativePointer(pointer)) {
		  ...
		  pointerUtils.getXY('client', pointer, page);
		}
